...
def run_tests() -> bool:
    ...
    h.run("AppContext 초기화 — 모든 서비스 생성", test_app_context_init)
    h.run("빈 상태 요약 — 전체 0값",              test_summary_empty)
    h.run("시료 등록 후 등록 시료 수 반영",        test_summary_sample_count)
    h.run("재고 변경 후 총 재고 반영",             test_summary_total_stock)
    h.run("주문 후 전체 주문 수 반영",             test_summary_total_orders)
    h.run("REJECTED 는 전체 주문에서 제외",        test_summary_rejected_excluded)
    h.run("생산 큐 WAITING 건수 반영",             test_summary_waiting_count)
    h.run("주문 상태별 카운트 반영",               test_summary_status_counts)
    h.run("종료 확인 — Y/y 만 긍정",              test_is_yes)
...
def test_app_context_init() -> None:
    ctx = _make_ctx()
...
def test_summary_sample_count() -> None:
    ctx = _make_ctx()
...
def test_summary_total_stock() -> None:
    ctx = _make_ctx()
    ctx.sample_service.register("A-001", "갈륨비소", 45.0, 0.92)
    ctx.sample_service.register("A-002", "실리콘",   30.0, 0.88)
...
def test_summary_total_orders() -> None:
    ctx = _make_ctx()
    ctx.sample_service.register("A-001", "갈륨비소", 45.0, 0.92)
    ctx.sample_service.update_stock("A-001", 100)
...
def test_summary_rejected_excluded() -> None:
    ctx = _make_ctx()
    ctx.sample_service.register("A-001", "갈륨비소", 45.0, 0.92)
    ctx.sample_service.update_stock("A-001", 100)
    ctx.order_service.place_order("고객1", "A-001", 5)
...
def test_summary_waiting_count() -> None:
    ctx = _make_ctx()
    ctx.sample_service.register("A-001", "갈륨비소", 45.0, 0.92)
